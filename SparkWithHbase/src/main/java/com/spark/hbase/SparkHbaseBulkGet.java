...
//		hconf.addResource(new Path("/etc/hbase/conf.cloudera.hbase/hbase-site.xml"));
	...
	public static class ResultFunction implements Function<Result,CountryBean>{
		...
		public CountryBean call(Result result) throws Exception {
			...
			cbean.setIp(Bytes.toString(result.getValue(Bytes.toBytes("logFields"),Bytes.toBytes("ip"))));
			cbean.setCountry(Bytes.toString(result.getValue(Bytes.toBytes("logFields"),Bytes.toBytes("country"))));
			cbean.setDescrip(Bytes.toString(result.getValue(Bytes.toBytes("logFields"),Bytes.toBytes("descrip"))));
			cbean.setLeader(Bytes.toString(result.getValue(Bytes.toBytes("logFields"),Bytes.toBytes("leader"))));
			cbean.setGDP(Bytes.toString(result.getValue(Bytes.toBytes("logFields"),Bytes.toBytes("GDP"))));
			return cbean;
		}
